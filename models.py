...
class MarketFactor(BaseModel):
    ...
    impact: str = Field(description="Explanation of how this factor affect the coin price or sentiment")
...
class CryptoAnalysis(BaseModel):
    ...
    sentiment: Literal["bullish", "bearish", "neutral"] = Field(description="Overall market sentiment for the coin based on current data")
    key_factors: List[MarketFactor] = Field(description="List of major market sentiment factors that are currently influencing this cryptocurrency")
    insights: List[CryptoInsights] = Field(description="List of insights and predictions for this coin with confidence scores)")
